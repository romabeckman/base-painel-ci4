...
class AuthValidation {
    ...
    function auth_current_password(string $str, string &$error = null) {
        ...
        $user = ServicesAuth::authRepository()->userModel->find(Auth::$user->id);
        return ServicesAuth::authHmac()->validateHash($str, $user->password);
    }

    function auth_strong_password(string $str, string &$error = null) {
        $error = lang('Auth.invalid_strong_password');
        return preg_match('/^\S*(?=\S{8,})(?=\S*[a-z])(?=\S*[A-Z])(?=\S*[\d])(?=\S*[\W])\S*$/', $str);
    }
}
